...
export async function POST(req: NextRequest) {
  ...
  const entry = {id:crypto.randomUUID(),site:site??'AgentLogs',type:type??'General',rating,message:message.trim(),email:email?.trim()||null,page:page??'/',createdAt:new Date().toISOString()}
